...
class BlogApp(wx.Frame):
    def __init__(self, *args, **kwargs):
        ...
        # Tags Sidebar
        self.tree = wx.TreeCtrl(self.panel)  # Initialize the tree control
        ...
        tags_sizer.Add(self.tree, 1, wx.EXPAND | wx.ALL, 5)  # Add the tree control to the sizer
        ...
        main_sizer.Add(tags_sizer, 2, wx.EXPAND | wx.ALL, 5)  # Ensure the tags sizer is added to the main sizer
        self.panel.SetSizer(main_sizer)
    ...
    def RefreshTagHierarchy(self):
        ...
        def AddTagToTree(tag_id, tag_name, parent_item):
            ...
            print(f"Added tag: {tag_name} under parent: {self.tree.GetItemText(parent_item)}")  # Debug statement
            conn = sqlite3.connect("notes.db")
            cursor = conn.cursor()
    ...
    def OnAddTag(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            self.RefreshTagHierarchy()  # Refresh the tag tree after adding a tag
        dialog.Destroy()
    ...
    def SaveTagToDatabase(self, tag_data, note_id):
        ...
        try:
            ...
            cursor.execute("INSERT OR IGNORE INTO Tags (name, parent_id) VALUES (?, ?)",
                           (tag_data["name"], tag_data["parent_id"] if tag_data["parent_id"] != -1 else None))
            conn.commit()
