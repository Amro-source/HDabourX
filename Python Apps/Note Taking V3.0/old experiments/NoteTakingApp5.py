...
class TreeApp(wx.Frame):
    def __init__(self, parent, title):
        ...
        notes_sizer = wx.BoxSizer(wx.HORIZONTAL)  # Change from wx.VERTICAL to wx.HORIZONTAL

        # Right Side: Tags List
        tags_panel = wx.Panel(notes_panel)
        tags_sizer = wx.BoxSizer(wx.VERTICAL)

        self.tags_list = wx.ListCtrl(tags_panel, style=wx.LC_REPORT)
        self.tags_list.InsertColumn(0, "Tag ID", width=50)
        self.tags_list.InsertColumn(1, "Tag Name", width=150)

        self.add_tag_btn = wx.Button(tags_panel, label="Add Tag to Note")

        tags_sizer.Add(wx.StaticText(tags_panel, label="Tags for Selected Note"), 0, wx.ALL, 5)
        tags_sizer.Add(self.tags_list, 1, wx.EXPAND | wx.ALL, 5)
        tags_sizer.Add(self.add_tag_btn, 0, wx.EXPAND | wx.ALL, 5)

        self.add_tag_btn.Bind(wx.EVT_BUTTON, self.on_add_tag_to_note)
        ...
        notes_sizer.Add(tags_panel, 1, wx.EXPAND | wx.ALL, 5)
    ...
    def on_add_tag_to_note(self, event):
        """Add a tag to the selected note."""
        ...
        dialog = wx.TextEntryDialog(self, "Enter Tag Name:", "Add Tag")
        if dialog.ShowModal() == wx.ID_OK:
            tag_name = dialog.GetValue().strip()
            if tag_name:
                conn = sqlite3.connect("notes.db")
                cursor = conn.cursor()
                ...
                self.refresh_tags(note_id)
                wx.MessageBox(f"Tag '{tag_name}' added to the note!", "Success")
        dialog.Destroy()
    ...
    def refresh_tags(self, note_id):
        """Refresh the tags list for the selected note."""
        ...
        for tag_id, tag_name in cursor.fetchall():
            self.tags_list.Append([str(tag_id), tag_name])
        conn.close()
