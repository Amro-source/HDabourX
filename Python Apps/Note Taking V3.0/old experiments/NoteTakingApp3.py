...
class TreeApp(wx.Frame):
    ...
    def on_add_note(self, event):
        """Add a new note with both title and content."""
        title_dialog = wx.TextEntryDialog(self, "Enter Note Title:", "Add Note")
        if title_dialog.ShowModal() == wx.ID_OK:
            title = title_dialog.GetValue().strip()
            if title:
                content_dialog = wx.TextEntryDialog(self, "Enter Note Content:", "Add Note")
                if content_dialog.ShowModal() == wx.ID_OK:
                    content = content_dialog.GetValue().strip()
                    conn = sqlite3.connect("notes.db")
                    cursor = conn.cursor()
