...
class TableViewer(wx.Frame):
    ...
    def setup_tags_tab(self):
        ...
        # Tags ListBox for multi-selection
        self.tags_list = wx.ListBox(self.tags_tab, style=wx.LB_MULTIPLE)
        self.refresh_tags()

        # Notes under selected tags
        ...
        sizer.Add(wx.StaticText(self.tags_tab, label="Notes under selected tags:"), 0, wx.ALL, 5)
        ...
        self.tags_list.Bind(wx.EVT_LISTBOX, self.on_tags_selected)
    ...
    def refresh_tags(self, event=None):
        ...
        self.tag_name_to_id = {}  # Dictionary to map tag names to IDs
        conn = sqlite3.connect("notes.db")
        cursor = conn.cursor()
        cursor.execute("SELECT id, name FROM Tags")
        for row in cursor.fetchall():
            tag_id, tag_name = row
            self.tags_list.Append(tag_name)  # Append tag names to the list
            self.tag_name_to_id[tag_name] = tag_id  # Map tag name to ID
        conn.close()
    ...
    def on_tags_selected(self, event):
        ...
        selected_indices = self.tags_list.GetSelections()
        if not selected_indices:
            return

        # Retrieve selected tag IDs using the mapping dictionary
        selected_tag_ids = [
            str(self.tag_name_to_id[self.tags_list.GetString(i)]) for i in selected_indices
        ]

        conn = sqlite3.connect("notes.db")
        cursor = conn.cursor()

        # Prepare the SQL query to fetch notes associated with selected tags
        query = '''
        SELECT Notes.id, Notes.title, Notes.content 
        FROM Notes
        JOIN NoteTags ON Notes.id = NoteTags.note_id
        WHERE NoteTags.tag_id IN ({})
        '''.format(','.join('?' * len(selected_tag_ids)))

        cursor.execute(query, selected_tag_ids)

        # Append the results to the notes list
        for row in cursor.fetchall():
