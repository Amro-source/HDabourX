...
class BlogApp(wx.Frame):
    ...
    def RefreshTags(self, note_id):
        ...
        WHERE NoteTags.note_id = ?''', (note_id,))
        for row in cursor.fetchall():
    ...
    def OnAddTag(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            tag_name = dialog.GetTagName().strip()
            if not tag_name:
                wx.MessageBox("Tag name cannot be empty.", "Warning")
                return

            note_id = self.notes_list.GetItemText(selected_note, col=0)
            self.SaveTagToDatabase(tag_name, note_id)
        dialog.Destroy()

    def SaveTagToDatabase(self, tag_name, note_id):
        conn = sqlite3.connect("notes.db")
        cursor = conn.cursor()
        try:
    ...
    def OnDeleteNote(self, event):
        ...
        if selected_note != -1:
            ...
            conn.commit()
            conn.close()
            wx.MessageBox("Note deleted successfully!", "Info")
            self.RefreshNotes()
            self.tags_list.Clear()

    def OnDeleteTag(self, event):
        selected_note = self.notes_list.GetFirstSelected()
        selected_tag = self.tags_list.GetSelection()
        if selected_note != -1 and selected_tag != -1:
            note_id = self.notes_list.GetItemText(selected_note, col=0)
            tag_name = self.tags_list.GetString(selected_tag)
            conn = sqlite3.connect("notes.db")
            cursor = conn.cursor()
            cursor.execute("SELECT id FROM Tags WHERE name = ?", (tag_name,))
            tag_id = cursor.fetchone()[0]
            cursor.execute("DELETE FROM NoteTags WHERE note_id = ? AND tag_id = ?", (note_id, tag_id))
            conn.commit()
            conn.close()
            wx.MessageBox("Tag removed successfully!", "Info")
            self.RefreshTags(note_id)
