...
class BlogApp(wx.Frame):
    ...
    def OnNoteSelected(self, event):
        selected_note_id = self.notes_list.GetItemText(event.GetItem().GetId())
        self.RefreshTags(selected_note_id)
    ...
    def OnAddTag(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            self.SaveTagToDatabase(tag_name)
        dialog.Destroy()

    def SaveTagToDatabase(self, tag_name):
        conn = sqlite3.connect("notes.db")
        cursor = conn.cursor()
        try:
            ...
            wx.MessageBox("Tag already exists.", "Warning")
        conn.close()
