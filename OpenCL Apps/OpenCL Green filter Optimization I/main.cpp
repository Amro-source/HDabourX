...
cl_context init_opencl(cl_device_id& device, cl_command_queue& queue, cl_kernel& kernel) {
    ...
    __kernel void green_filter(__global uchar4* image, int width, int height) {
        int x = get_global_id(0);
        int y = get_global_id(1);
        if (x >= width || y >= height) return;
        
        int idx = y * width + x;
        image[idx].x = 0; // Zero out Blue
        image[idx].z = 0; // Zero out Red
    }
)";
...
void apply_green_filter_gpu(cv::Mat& frame, cl_context context, cl_command_queue queue, cl_kernel kernel) {
    ...
    if (err != CL_SUCCESS) {
        std::cerr << "Failed to map OpenCL buffer" << std::endl;
        return;
    }
    ...
    // Define optimal work size (aligned to warp size for better execution)
    size_t globalWorkSize[2] = { (size_t)frame.cols, (size_t)frame.rows };

    // Use event-based execution to avoid blocking calls
    cl_event kernel_event;
    err = clEnqueueNDRangeKernel(queue, kernel, 2, nullptr, globalWorkSize, nullptr,
    ...
    mappedPtr = (uchar*)clEnqueueMapBuffer(queue, buffer, CL_TRUE, CL_MAP_READ,
        ...
        0, nullptr, nullptr, &err);
    if (err != CL_SUCCESS) {
        std::cerr << "Failed to map OpenCL buffer for reading" << std::endl;
        return;
    }
