...
Original file is located at
    https://colab.research.google.com/drive/1246XGJbtbqQ4pb86tGk64hZAF3_bV6hM
...
class GAN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        z = torch.randn(batch_size, self.latent_dim).type_as(real_imgs)
        ...
        g_loss = self.criterion(fake_preds, valid)
        ...
        d_real_loss = self.criterion(real_preds, valid)

        fake_preds = self.discriminator(fake_imgs.detach())
        d_fake_loss = self.criterion(fake_preds, fake)
    ...
    def configure_optimizers(self):
        g_opt = optim.Adam(self.generator.parameters(), lr=0.0002, betas=(0.5, 0.999))
        d_opt = optim.Adam(self.discriminator.parameters(), lr=0.0002, betas=(0.5, 0.999))
        return [g_opt, d_opt]

    def generate_images(self, num_images=16):
        """Generate and plot fake images."""
        self.eval()  # Set model to evaluation mode
        z = torch.randn(num_images, self.latent_dim).to(self.device)  # Generate latent noise
        with torch.no_grad():
            fake_imgs = self.generator(z).cpu()

        fake_imgs = fake_imgs.view(-1, 28, 28).numpy()  # Reshape to 28x28
...
dm = MNISTDataModule()
model = GAN()
trainer = pl.Trainer(max_epochs=50, accelerator="gpu", devices=AVAIL_GPUS if AVAIL_GPUS > 0 else "auto")
...
model.generate_images(num_images=16)

import torch
import os

def save_models(generator, discriminator, checkpoint_dir="checkpoints"):
...
def load_models(generator, discriminator, checkpoint_dir="checkpoints"):
...
save_models(model.generator, model.discriminator)

load_models(model.generator, model.discriminator)