...
# Function to generate real tones (sine waves with optional harmonics)
def generate_real_tones(n_samples, min_freq=100, max_freq=2000, signal_length=44100, add_harmonics=False):
    ...
    for i in range(n_samples):
        ...
        if add_harmonics:
            harmonic_frequencies = [frequency * n for n in range(2, 5) if frequency * n < max_freq]
            for harmonic_freq in harmonic_frequencies:
                harmonic_amplitude = amplitude / (harmonic_freq / frequency)
                X[i] += harmonic_amplitude * np.sin(2 * np.pi * harmonic_freq * t + phase)
...
class ToneGeneratorApp(wx.App):
    def OnInit(self):
        ...
        self.num_samples_input = wx.TextCtrl(panel, value="1000")  # Default value: 1000 samples
        ...
        self.harmonics_checkbox = wx.CheckBox(panel, label="Add Harmonics")
        ...
        sizer.Add(self.harmonics_checkbox, 0, wx.ALL | wx.EXPAND, 5)
    ...
    def on_generate_and_save(self, event):
        ...
        try:
            ...
            if num_samples <= 0 or min_freq >= max_freq or min_freq <= 0:
        ...
        # Generate tones
        signal_length = 44100
        add_harmonics = self.harmonics_checkbox.IsChecked()
        tones = generate_real_tones(num_samples, min_freq=min_freq, max_freq=max_freq, signal_length=signal_length, add_harmonics=add_harmonics)
        ...
        wx.MessageBox(f"{num_samples} tones (frequency range: {min_freq}-{max_freq} Hz) generated and saved to {output_dir}.", "Success", wx.OK | wx.ICON_INFORMATION)
