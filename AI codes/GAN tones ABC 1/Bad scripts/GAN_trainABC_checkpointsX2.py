...
def abc_algorithm(generator, discriminator, real_data, latent_dim, n_employed, n_onlookers, n_scouts, limit, epochs):
    ...
    for epoch in range(start_epoch, epochs):
        for i in range(n_employed):
            candidate = perturb_state_dict(population[i], epoch)
        ...
        for i in range(n_onlookers):
            ...
            candidate = perturb_state_dict(population[idx], epoch)
            candidate_fitness = evaluate_fitness(generator, discriminator, real_data, latent_dim, candidate)
        ...
        for i in range(n_scouts):
            ...
            if np.random.rand() < 0.2:  # Increased probability for scouts
...
def perturb_state_dict(state_dict, epoch):
    perturbation_scale = 0.1 * (1 + epoch / epochs)  # Dynamic perturbation scale
    perturbed_state_dict = {}
    for key, value in state_dict.items():
        perturbed_state_dict[key] = value + torch.randn_like(value) * perturbation_scale
...
def evaluate_fitness(generator, discriminator, real_data, latent_dim, weights):
    ...
    real_output = discriminator(real_data + torch.randn_like(real_data) * 0.01).view(-1)  # Add noise to real data
    ...
    fitness = -(generator_loss.item() + 0.5 * discriminator_loss.item())  # Adjusted weight for discriminator loss
    return fitness
