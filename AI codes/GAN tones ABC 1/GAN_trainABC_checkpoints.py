...
def evaluate_fitness(generator, discriminator, real_data, latent_dim, weights):
    ...
    # Generate fake data
    noise = torch.randn(real_data.size(0), latent_dim, dtype=torch.float32).to(device)  # Match batch size of real data
    fake_data = generator(noise)

    # Discriminator output
    real_output = discriminator(real_data).view(-1)  # Flatten the output
    fake_output = discriminator(fake_data).view(-1)  # Flatten the output

    # Adversarial loss
    adv_loss = -torch.mean(torch.log(real_output + 1e-8) + torch.log(1 - fake_output + 1e-8))
    ...
    fitness = -(adv_loss.item() + 0.1 * spectral_l.item())
    return fitness
