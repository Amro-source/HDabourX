...
# Optimizers
optimizer_G = optim.Adam(generator.parameters(), lr=0.00001, betas=(0.5, 0.999))  # Reduced learning rate
...
def train_gan(generator, discriminator, optimizer_G, optimizer_D, latent_dim, epochs, batch_size, sample_interval, data_dir):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, num_samples, batch_size):
            ...
            d_loss.backward()
            torch.nn.utils.clip_grad_norm_(discriminator.parameters(), max_norm=10)  # Gradient clipping
            optimizer_D.step()
            ...
            g_loss.backward()
            torch.nn.utils.clip_grad_norm_(generator.parameters(), max_norm=10)  # Gradient clipping
            optimizer_G.step()
...
def plot_loss_curves(d_losses, g_losses):
    ...
    # Generator loss
    plt.subplot(1, 2, 2)
    plt.plot(g_losses, color='orange', label="Generator Loss")
