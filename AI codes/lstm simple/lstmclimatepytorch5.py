...
seq_length = 30  # Improved sequence length
...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size=50, num_layers=2, dropout=0.4):  # Increased dropout
...
loss_fn = nn.MSELoss()
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5)  # Reduce LR on plateau
...
y_test_pred_smoothed = np.convolve(y_test_pred_original.flatten(), np.ones(5)/5, mode='valid')
