...
# Create a scaler object for the target values
scaler_y = MinMaxScaler()

# Fit the scaler to the training target values and transform both the training and testing target values
...
optimizer = optim.Adam(list(lstm.parameters()) + list(dropout.parameters()) + list(fc.parameters()), lr=0.001)
loss_fn = nn.MSELoss()
...
y_pred_scaled = out.detach().numpy()
